...
void cleanup(int sig) {
    ...
    if (awww_pid > 0) kill(awww_pid, SIGTERM);
    exit(0);
}
  ...
  const char *display = getenv("WAYLAND_DISPLAY");
    ...
    if (argc > argi) {
        ...
        if (connect(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
            fprintf(stderr, "wwl: daemon not running on display %s (%s)\n",
                    display, sock_path);
            close(fd);
            return 1;
        }
        ...
        else { fprintf(stderr, "wwl: unknown command: %s\n", arg); close(fd); return 1; }
    ...
    pid_t awww_pid = fork();
    if (awww_pid == 0) {
    execlp("awww-daemon", "awww-daemon", "--no-cache", NULL);
    _exit(1);
    }
