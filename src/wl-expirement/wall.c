...
void wall_select(const void *arg) {
    ...
    if (is_wayland()) {
        /* Wayland: use fuzzel in dmenu mode */
        snprintf(cmd, sizeof(cmd), "seq 1 %d | fuzzel -d -p 'Wallpaper:'", count);
    } else {
        /* X11: use rofi in dmenu mode */
