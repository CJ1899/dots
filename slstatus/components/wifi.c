...
wifi_perc(const char *interface)
{
    ...
    /* 1. Check Operstate first for the icons */
    ...
    if (strncmp(status, "up", 2) == 0) {
        ...
        while (fgets(line, sizeof(line), fp)) {
            if (strstr(line, interface)) {
                ...
                /* We skip the interface name and the next integer (status) */
                char *p = strchr(line, ':');
                if (p && sscanf(p + 1, " %*d %d", &cur) == 1) {
                    break;
                }
            }
        }
    ...
    char flags[16] = {0};
    esnprintf(path, sizeof(path), "/sys/class/net/%s/flags", interface);
    if ((fp = fopen(path, "r"))) {
        fgets(flags, sizeof(flags), fp);
        fclose(fp);
        if (strstr(flags, "0x1003")) return "📡";
    }
...
nets_status(const char *unused)
{
    ...
    /* Ethernet Check (e*) */
    if ((fp = fopen("/sys/class/net/enp0s25/operstate", "r"))) {
        fgets(eth_status, 5, fp);
        fclose(fp);
    }

    /* VPN Check (tun0) */
    int vpn = 0;
    if ((fp = fopen("/sys/class/net/tun0/operstate", "r"))) {
        vpn = 1;
        fclose(fp);
    }

    return bprintf("%s%s",
        (strcmp(eth_status, "up\n") == 0 ? "🌐" : "❎"),
        (vpn ? " 🔒" : ""));
}
	const char *
